...
holes_positions_direction_magnet_faces_y = cell(11, 10);

for x_pos = 1:13
    for y_pos = 1:12
        holes_positions_direction_magnet_faces_y{x_pos, y_pos} = struct('x', 30e-3 + (x_pos - 1) * 15e-3, 'y',  37.5e-3 + (y_pos - 1) * 15e-3, 'z', 50e-3 + 2e-3 + Radius);
    end
end
