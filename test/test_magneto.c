...
int main(int argc, char **argv) {
	...
	FILE *fp;
	fp = fopen(argv[1], "r");
	while (fgets(buf, 100, fp) != NULL) nlines++;
	rewind(fp);
	...
	for (i = 0; i < 36; i++) SS[i] = S11[i] - S22b[i];
	...
	for (i = 0; i < 9; i++) A_1[i] = SQ[i] * hm / hmb;
