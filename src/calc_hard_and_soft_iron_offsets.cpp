...
std::generator<std::tuple<double, double, double>> file_mag_data_generator(std::istream&& stream) {
	for (double x, y, z; stream >> common::ignore_literal<int>() >> common::chlit(',') >> x >> common::chlit(',') >> y >> common::chlit(',') >> z;) {
...
std::tuple<Eigen::Vector<double, 3>, Eigen::Matrix<double, 3, 3>> compute_offsets(Eigen::Vector<double, 10> const& v, double norm_of_magnetic_field) {
	...
	double abc = B.transpose() * Q * B - v(9);
	common::println(abc);
	common::println(- v(9));
	common::println(B.transpose() * Q * B);
	Eigen::DiagonalMatrix<double, 3> D = (solver.eigenvalues().real() * (1 / abc)).cwiseSqrt().asDiagonal();
...
int main() {
	...
	{
		std::ifstream file(std::filesystem::path(CMAKE_SOURCE_DIR) / "result" / filename);

		Eigen::Matrix<double, Eigen::Dynamic, 3> data;
		unsigned int row = 0;
		for (double x, y, z; file >> common::ignore_literal<unsigned int>() >> common::chlit(',') >> x >> common::chlit(',') >> y >> common::chlit(',') >> z; ++row) {
	...
	}
	{
		...
		std::vector<double> y;
		std::vector<double> z;
		for (double x_e, y_e, z_e; file >> common::ignore_literal<int>() >> common::chlit(',') >> x_e >> common::chlit(',') >> y_e >> common::chlit(',') >> z_e;) {
			x.push_back(x_e);
			y.push_back(y_e);
			z.push_back(z_e);
		}
